...
module Firmata
  ...
  class BoardTest < MiniTest::Unit::TestCase
    ...
    def test_writing_report_version
      mock_sp = mock_serial_port(Board::REPORT_VERSION)
    ...
    def test_processing_report_version
      board = Board.new(FakeSerialPort.new)
      board.report_version
    ...
    def test_writing_capability_query
      mock_sp = mock_serial_port(Board::START_SYSEX, Board::CAPABILITY_QUERY, Board::END_SYSEX)
    ...
    def test_writing_analog_mapping_query
      mock_sp = mock_serial_port(Board::START_SYSEX, Board::ANALOG_MAPPING_QUERY, Board::END_SYSEX)
    ...
    def test_processing_digital_message
      ...
      pin = board.pins[8]
      pin.mode = Board::INPUT

      
      board.send :process, "\x91\x01\x00"

      assert_equal Board::HIGH, pin.value
    end

    def test_set_pin_mode
      mock_sp = mock_serial_port(Board::PIN_MODE, 13, Board::OUTPUT)

      board = Board.new(mock_sp)
      board.pins[13] = Board::Pin.new([0, 1, 4], 0, 0, nil)

      board.set_pin_mode(13, Board::OUTPUT)

      assert_equal Board::OUTPUT, board.pins[13].mode
      mock_sp.verify
    end

    def test_write_pin_state_query
      mock_sp = mock_serial_port(Board::START_SYSEX, Board::PIN_STATE_QUERY, 13, Board::END_SYSEX)
    ...
    def test_processing_pin_state_query
      ...
      board.pins[13].mode = Board::INPUT
    ...
    def test_reset
      mock_sp = mock_serial_port(Board::SYSTEM_RESET)
    ...
    def test_digital_write
      ...
      8.times do |x|
        board.pins[x + 8] = Board::Pin.new([], 0, 250 + x, nil)
      end
